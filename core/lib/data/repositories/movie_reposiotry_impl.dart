...
class MovieRepositoryImpl implements MovieRepository {
  ...
  @override
  Future<Either<Failure, String>> removeWathclist(MovieDetail movie) async {
    try {
      final result =
  ...
  //---Series---
  @override
  Future<Either<Failure, List<Serie>>> getOnTheAirSeries() async {
    if (await networkInfo.isConnected) {
      try {
        final result = await remoteDataSource.getOnTheAirSeries();
        localDataSource.cacheOnTheAirSeries(
            result.map((serie) => SeriesTable.fromDTO(serie)).toList());
        return Right(result.map((model) => model.toEntity()).toList());
      } on ServerException {
        return const Left(ServerFailure(''));
      } on SocketException {
      ...
      try {
        final result = await localDataSource.getCachedOnTheAirSeries();
        return Right(result.map((model) => model.toEntity()).toList());
      } on CacheException catch (e) {
        return Left(CacheFailure(e.message));
      }
  ...
  @override
  Future<Either<Failure, List<Serie>>> getSeriesRecommendations(int id) async {
    try {
  ...
  @override
  Future<Either<Failure, List<Serie>>> getPopularSeries() async {
    try {
  ...
  @override
  Future<Either<Failure, List<Serie>>> getTopRatedSeries() async {
    try {
  ...
  @override
  Future<Either<Failure, List<Serie>>> searchSeries(String query) async {
    try {
  ...
  @override
  Future<Either<Failure, String>> removeWathclistSeries(
      SeriesDetail movie) async {
    try {
      final result = await localDataSource
          .removeWathclistSeries(SeriesTable.fromEntity(movie));
      return Right(result);
    } on DatabaseException catch (e) {
      return Left(DatabaseFailure(e.message));
    }
  ...
  @override
  Future<Either<Failure, List<Serie>>> getWatchlistSeries() async {
