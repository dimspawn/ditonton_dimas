import 'package:dartz/dartz.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mockito/mockito.dart';
import 'package:series/domain/usecases/remove_watchlist_series.dart';

import '../../dummy_data/dummy_objects.dart';
import '../../helpers/test_helpers.mocks.dart';

void main() {
  late RemoveWatchlistSeries usecase;
  late MockMovieRepository mockMovieRepository;

  setUp(() {
    mockMovieRepository = MockMovieRepository();
    usecase = RemoveWatchlistSeries(mockMovieRepository);
  });

  test('should remove watchlist serie from repository', () async {
    // arrange
    when(mockMovieRepository.removeWathclistSeries(testSerieDetail))
        .thenAnswer((_) async => const Right('Removed from watchlist'));
    // act
    final result = await usecase.execute(testSerieDetail);
    // assert
    verify(mockMovieRepository.removeWathclistSeries(testSerieDetail));
    expect(result, const Right('Removed from watchlist'));
  });
}
